...
var loadreport = {
    ...
    performance: {
        ...
        onLoadFinished: function (page, config, status) {
            ...
            if(phantom.args.indexOf('csv') >= 0){
                this.printToFile(config,report,'loadreport','csv');
            }

            if(phantom.args.indexOf('json') >= 0){
                this.printToFile(config,report,'loadreport','json');
            }

            if(phantom.args.indexOf('junit') >= 0){
                this.printToFile(config,report,'loadreport','xml');
            }
    ...
    processArgs: function (config, contract) {
        ...
        contract.forEach(function(argument) {
            if (a < phantom.args.length) {
                ...
                //console.log("Setting arguement " + config[argument.name] + " to: " + phantom.args[a]);
            } else {
                if (argument.req) {
    ...
    printToFile: function(config,report,filename,extension) {
        var f, myfile,
            keys = [], values = [];
       ...
       if(phantom.args[4]){ 
            myfile = phantom.args[3] + '/' + filename + '-' + phantom.args[4] + '.' + extension;
	   } else if(phantom.args[3]) {
            myfile = phantom.args[3] + '/' + filename + '.' + extension;
       }else{
            myfile = 'reports/' + filename + '.' + extension;

       }

        if(!this.config.alwaysNew && fs.exists(myfile)){
